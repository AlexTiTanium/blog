---
title: "Spark: подивився «Чорнобиль» і сів писати ігровий рушій"
date: "2026-05-14"
...
Просто клепати гру мені нудно. Хочеться заразом поекспериментувати з ШІ й самому чогось навчитися. Я вже підступався до цього раніше (рушій на Rust), то чому б його не воскресити. Перший проєкт мені свого часу набрид рівно тієї миті, коли дійшло до рендера: я вперся в купу проблем з ECS Shipyard. Її ніколи не було розраховано на те, щоб її так використовували.

## Моя геніально тупа ідея

І тут вмикається мій улюблений жанр ідей — геніально тупі, як завжди. А що, як викинути майже весь рушій разом з усім його API і замінити одним-єдиним ECS? Звучить дико, знаю.

Хай усе в рушії буде однією з трьох речей. Компоненти — їх багато. Ресурси — вони в єдиному екземплярі. Системи — це вся логіка. А далі найвеселіше: рендер, асет-менеджер, камера, звук — це теж просто ресурси. І будь-яка система, якій вони потрібні, просто запитує їх за іменем. Один світ, одні правила для всього.

Раз кожна система заздалегідь каже, що читає і що пише, я можу зібрати планувальник, який розкидає по потоках таски, що не перетинаються, і все закрутиться паралельно саме собою. Власне, тому ECS у мене й став серцем рушія, а не просто однією з деталей.

У коді це виглядає приблизно так. Жодного «обʼєкта рушія» з методами — є `World`, і в ньому живе все:
...
Логічне питання: навіщо городити своє, якщо є [Bevy](https://github.com/bevyengine/bevy/tree/main/crates/bevy_ecs) і [Shipyard](https://github.com/leudz/shipyard). Мені дуже подобається синтаксис Bevy — він помітно логічніший за Shipyard. Але в Shipyard є ворклоадс (workloads), і ось вони зроблені чудово. І я ніяк не можу вибрати.

При цьому Bevy величезний. А Shipyard — ті ще нетрі. І якщо мені знадобиться щось, чого там немає і що не підтримується, я це з жодним ШІ не витягну: я ж навіть приблизно не уявляю, як ці штуки влаштовані всередині. Що, власне, і є справжня причина всього задуму — я хочу розуміти. Хоча б на рівні структур і тих рішень, які ухвалюють автори таких бібліотек. За рахунок чого воно працює так швидко. На які компроміси вони йдуть.
...
## Подивимося, на якому місці я зламаюся
